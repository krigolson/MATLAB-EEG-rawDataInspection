function doRawDataInspection(EEG)
    ...
    txt4 = uicontrol('Style','text','Position',[scrsz(3)-825 bottom+60 200 20],'String',message4,'HorizontalAlignment','left');
    txt5 = uicontrol('Style','text','Position',[scrsz(3)-825 bottom+80 200 20],'String',message5,'HorizontalAlignment','left');
    ...
    function drawPlots(source,event)
        ...
        cla(f3);
        imagesc(OUTEEG.artifact(2).artifactSize',[0 EEG.artifactCriteriaGradient]);
        xticks([1:1:length(EEG.chanlocs)]);
        xtickangle(90);
        ...
        cla(f4);
        imagesc(OUTEEG.artifact(1).artifactSize',[0 EEG.artifactCriteriaDifference]);
        xticks([1:1:length(EEG.chanlocs)]);
        xtickangle(90);
    ...
    function redoMath
        ...
        OUTEEG = doArtifactRejection(OUTEEG,'Difference',EEG.artifactCriteriaDifference);
        OUTEEG = doArtifactRejection(OUTEEG,'Gradient',EEG.artifactCriteriaGradient);
        EEG.percentDifference = sum(OUTEEG.artifact(1).badSegments,2)/EEG.totalMarkers*100;
        EEG.percentGradient = sum(OUTEEG.artifact(2).badSegments,2)/EEG.totalMarkers*100;
        EEG.rejectedDifference = max(sum(OUTEEG.artifact(1).badSegments,2));
        EEG.rejectedGradient = max(sum(OUTEEG.artifact(2).badSegments,2));
        ...
        for counter = 1:size(EEG.data,1)
            ciS = makeCIs(allVariances(counter,:));
            EEG.CIs(counter,:) = ciS(3,:);
        end
        EEG.meanVariances = [];
